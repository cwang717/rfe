...
\textbf{Category} & \textbf{Evidence and Significance} \\
\midrule
\textbf{Framework extension} & \textbf{Li et al. (2022), \ExhibitRef{5D-1}.} Used my first-authored review as a foundation for a later lane-changing method that considered not only the autonomous vehicle's own path, but also its effect on nearby vehicles and traffic flow. \\
\addlinespace[0.2em]
\textbf{Public-sector / federal use} & \textbf{Federal Highway Administration project report --- Soleimaniamiri et al. (2021), FHWA-HRT-21-068, \ExhibitRef{5D-2}.} Cited my work \textit{Trajectory Optimization for a Connected Automated Traffic Stream: Comparison Between an Exact Model and Fast Heuristics} in a federal CARMA proof-of-concept report involving trajectory smoothing, trajectory planning, trajectory control, signal optimization, and dynamic lane assignment. \\
\addlinespace[0.2em]
\textbf{National-laboratory technical extension} & \textbf{Zhou et al. (2024), Oak Ridge National Laboratory, \ExhibitRef{5D-3}.} Cited my proactive longitudinal control / disruptive lane-change control papers in later research on learning-based car-following control, behavior cloning, deep reinforcement learning, and generalizability under stop-and-go traffic. \\
\addlinespace[0.2em]
\textbf{Structure-specific extension} & \textbf{Zhuo et al. (2023), Nanyang Technological University, Singapore, \ExhibitRef{5D-4}.} Used my \textit{Cooperative Roundabout Control Strategy for Connected and Autonomous Vehicles} in later research on CAV platooning configurations at isolated roundabouts in mixed traffic. \\
\addlinespace[0.2em]
\textbf{Patent translation} & \textbf{U.S. Patent No. 11,697,435 B1 --- Hierarchical Vehicle Action Prediction, \ExhibitRef{5D-5}.} Shows that my autonomous-driving prediction and interaction-aware planning work was translated into a protected technical invention for predicting hierarchical vehicle actions and supporting autonomous driving decisions in interaction-heavy scenarios such as merging and surrounding-vehicle behavior prediction. \\
\bottomrule
\end{tabularx}
\end{singlespace}
